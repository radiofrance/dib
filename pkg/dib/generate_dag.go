...
func GenerateDAG(buildPath, registryPrefix, customHashListPath string, buildArgs map[string]string) (*dag.DAG, error) {
	...
	if err := computeGraphHashes(graph, customHashList, buildArgs); err != nil {
...
func computeGraphHashes(graph *dag.DAG, customHashList []string, buildArgs map[string]string) error {
